...
### [2026-07-19] — Spec 014: modelo con autoría en la API/app + selector de autor (APROBADA)
- **Tipo**: decisión
- **Qué**: se aprobó `specs/014-modelo-autor-en-app/spec.md`. Alcance en 4 partes: (A) serializar el modelo ganador de la spec 013 (LGBM por defecto, Escenario A) con un notebook nuevo estilo `STG_8.2` — artefactos con **nombres nuevos** (sufijo `_autor`), sin pisar nada de las specs 009/010; (B) unificar la nómina a **257 diputados** de raíz (fusionar los duplicados por grafía OSUNA, Blanca Inés y PICHETTO, Miguel Angel, conservando bloque/provincia actuales y **combinando** el historial de ambas grafías, no descartando una fila); (C) `POST /predecir` pasa a exigir **título + autor obligatorio** (257 diputados con su bloque actual, o Poder Ejecutivo Nacional → coalición gobernante vigente derivada de `tabla_periodos_presidenciales.csv`, hoy La Libertad Avanza) y arma las features de autoría con la definición oficial del Escenario A (PRO fuera del oficialismo); (D) la app suma el selector de autor (258 opciones) y muestra autor/bloque asignado en el resultado.
- **Decisiones tomadas con el usuario (AskUserQuestion)**: (1) autor **obligatorio** — sin opción "sin autor" (el modelo soporta SIN_DATO si se quisiera a futuro); (2) la corrección 259→257 aplica a **toda la nómina** (selector, `/diputados` y `/predecir`), cerrando la deuda conocida desde la spec 010; (3) artefactos con **nombres nuevos**, no se pisan los existentes (Principio 5, permite volver atrás).
- **Corrección del usuario**: completar los 492 títulos sin autor **deja de ser deuda pendiente** — son casi todos pre-2019 (no son filas de entrenamiento del modelo actual; en 2019+ solo quedan 342 filas SIN_DATO, ~1,7%, que el modelo ya maneja).
- **Riesgos anotados para /planificar**: bloque actual de un diputado que `encoder_bloque_autor` nunca vio como autor; fuente confiable del bloque vigente (no "bloque del último voto" a ciegas); consistencia silenciosa entrenamiento–predicción (columnas/orden/normalización/dtype — recordar bug float32/float64 de la spec 010; lógica del vector aislada en una sola función); cambio incompatible del contrato de `/predecir` (app y API se actualizan juntas); recalcular las tasas históricas de Osuna/Pichetto sobre el historial combinado.
- **Impacto**: `specs/014-modelo-autor-en-app/spec.md` (nuevo). Próximo paso: `/planificar`.

### [2026-07-19] — Spec 014 COMPLETA: modelo con autoría en la API/app + selector de autor
- **Tipo**: decisión
- **Qué**: se cerraron las 17 tareas de `specs/014-modelo-autor-en-app/tasks.md` y los 11 criterios de aceptación de `spec.md`. Resultado:
  - **Notebook nuevo** `notebooks/STG_8.2_serializar_artefactos_autor.ipynb` (T1-T8): genera `modelo_lgbm_autor.joblib` (LGBM `class_weight='balanced'`, `random_state=42`, Escenario A, reentrenado sobre las 20.608 filas de `df_entrenamiento_autor.csv`), `orden_features_autor.json` (398 features), `nomina_diputados.csv` (257 canónicos), y 3 snapshots `_autor` con historial combinado. Ninguno de los artefactos de las specs 009/010 fue tocado. Reproducibilidad verificada por hash SHA-256 en dos corridas completas.
  - **API** (`api/database.py`, `api/schemas.py`, `api/modelo.py`, routers): `GET /diputados` devuelve 257 (antes 259); `POST /predecir` exige `{titulo, autor}` y arma las 4 features de autoría del Escenario A; `construir_features(titulo, autor)` sigue siendo la única función que arma el vector.
  - **App** (`app/app.py`): selector de autor con 258 opciones (Poder Ejecutivo Nacional + 257 diputados con bloque actual en la etiqueta); resultado muestra autor y bloque asignado.
  - **Chequeo de sanidad del modelo reentrenado**: F1-macro = 0.928 sobre el holdout de STG_6.2 (≥ 0.581 esperado, confirma que la serialización no rompió nada). **La métrica oficial para citar sigue siendo 0.581** (holdout genuino de STG_6.2, el modelo reentrenado ya vio esas filas).
- **Hallazgos/decisiones de diseño durante la implementación**:
  - **`FEATURES_A` no sigue el orden natural de columnas del CSV**: en STG_6.2, `es_oficialista`/`coincide_bloque_autor` se agregan al final de la lista, no en su posición dentro de `df_entrenamiento_autor.csv`. Si `orden_features_autor.json` se hubiera reconstruido leyendo el encabezado del CSV en vez de reproducir textualmente la expresión de STG_6.2, la API habría desalineado esas 2 columnas contra lo que el modelo aprendió — sin ningún error visible, solo prediciendo mal en silencio. Se evitó serializando el orden como artefacto propio en T5, verificado una a una contra las columnas de entrenamiento.
  - **`coincide_bloque_autor` es asimétrico según el autor**: para un autor legislador es comparación de texto (`bloque_votante == bloque_autor`); para el Poder Ejecutivo es **membresía en el set de `bloques_integrantes`** de la coalición vigente, no comparación de texto contra un único bloque. Este detalle solo se detectó leyendo el código original de `STG_5.2` (spec 011) antes de escribir `api/modelo.py`; replicarlo mal habría dado `coincide_bloque_autor` incorrecto para todo autor=PEN.
  - **48 de los 257 diputados actuales pertenecen a 14 bloques que `encoder_bloque_autor` nunca vio como autor** en el entrenamiento (el mayor: Provincias Unidas, 18 diputados). Comportamiento verificado como esperado: el encoder devuelve -1 (`unknown_value=-1`) sin romper la predicción; las otras 3 features de autoría (calculadas por texto) conservan la señal política igual. Probado explícitamente con un diputado de Provincias Unidas como autor.
  - **Caso Alí "Pipi"**: `diputados_actuales.csv` tiene el campo `Nombre` de "Alí, Ernesto "Pipi"" mal escapado como CSV (comillas internas sin el escape `""`), lo que dejaba `Ernesto Pipi""` en vez de `Ernesto "Pipi"`. Sin quitar comillas de la clave de cruce, este único diputado no matcheaba contra el dataset de votos (256/257 en vez de 257/257). Se resolvió quitando caracteres de comillas solo en la clave de emparejamiento, no en el nombre canónico mostrado.
  - **`GET /diputados` cambió de `list[str]` a `list[{diputado, bloque}]`** (agregado `DiputadoResumen` a `api/schemas.py`) para que la app pudiera mostrar el bloque en el selector de autor sin hacer 257 llamadas HTTP extra. Sigue devolviendo 257 elementos.
  - **Limpieza de código muerto**: los loaders viejos `cargar_snapshot_diputado()`/`_tema()`/`_bloque_tema()` (259 diputados, modelo de la spec 009) se eliminaron de `api/database.py` al confirmar que ya nadie los usaba — la API migró por completo al modelo nuevo, no quedaron ambos modelos convivendo en el código.
- **Bug recurrente (ya documentado, reapareció)**: al insertar celdas con `NotebookEdit`, pasar `\n` como texto literal (en vez de saltos de línea reales) generó `SyntaxError: unexpected character after line continuation` en la primera celda de T1. Misma causa raíz que la entrada del 2026-07-13 (saltos de línea mal escapados dentro del código), esta vez en la herramienta de edición de notebooks en lugar de en el código pegado. Se corrigió reescribiendo la celda con saltos de línea reales. **Lección**: al usar `NotebookEdit`, el parámetro `new_source` debe llevar saltos de línea reales, nunca la secuencia de escape `\n`.
- **Validado (T16)**: los 11 criterios de aceptación de `spec.md` marcados con evidencia (tarea + prueba concreta que lo verificó, incluyendo verificación end-to-end en el navegador con `uvicorn`+Streamlit reales).
- **Impacto**: notebook nuevo `notebooks/STG_8.2_serializar_artefactos_autor.ipynb`. Artefactos nuevos en `data/`: `modelo_lgbm_autor.joblib`, `orden_features_autor.json`, `nomina_diputados.csv`, `snapshot_diputado_autor.csv`, `snapshot_diputado_tema_autor.csv`, `snapshot_bloque_tema_autor.csv`. Modificados: `api/database.py`, `api/schemas.py`, `api/modelo.py`, `api/routers/diputados.py`, `api/routers/predecir.py`, `app/app.py`. **Próximo paso**: ninguno pendiente dentro de esta spec — recordar correr `/revisar`. Candidatos a spec futura (sin urgencia): deploy en Render/Streamlit Cloud, base de datos persistente, login/autenticación (checklist de la cátedra).

### [2026-07-19] — Spec 014: /revisar con sign-off + 2 correcciones cosméticas aplicadas
- **Tipo**: bug
- **Qué**: `/revisar` dio sign-off a la spec 014 (11/11 criterios cumplidos, Constitución 8/8) con dos hallazgos menores en `api/modelo.py`, corregidos de inmediato: (1) la constante `TARGET = "voto"` había quedado sin uso tras el rewrite de la Parte B (la versión vieja de `_orden_features()` la usaba para excluir columnas del CSV de entrenamiento; la nueva `_orden_features_autor()` lee directamente el JSON serializado y nunca la referencia) — se eliminó; (2) `_periodo_vigente()` solo registraba advertencia en el log cuando "hoy" cae después del último período conocido, no cuando cae antes del primero — se agregó la rama simétrica (aunque el escenario es extremo: requeriría el reloj del servidor puesto antes de 1989).
- **Por qué / causa raíz**: (1) es un resto de refactor, no afecta el comportamiento. (2) faltaba simetría en la función: el chequeo original solo cubría "la tabla quedó vieja" (futuro sin actualizar) pero no "el reloj está mal" (fecha en el pasado remoto) — mismo mecanismo de log, rama faltante.
- **Impacto**: `api/modelo.py` (2 cambios chicos). Verificado con `TestClient` que `/predecir` y `/diputados` siguen funcionando igual (257 predicciones, `bloque_autor` correcto). **Spec 014 queda completa y revisada, sin observaciones pendientes.**

### [2026-07-19] — Plan 014 (APROBADO): STG_8.2 con artefactos `_autor` + orden de features serializado
- **Tipo**: decisión
- **Qué**: se aprobó `specs/014-modelo-autor-en-app/plan.md`. Diseño: notebook nuevo `STG_8.2_serializar_artefactos_autor.ipynb` que genera 6 artefactos con nombres nuevos — `modelo_lgbm_autor.joblib` (LGBM `class_weight='balanced'`, `random_state=42`, reentrenado con todas las filas de `df_entrenamiento_autor.csv`, features Escenario A), `orden_features_autor.json`, `nomina_diputados.csv` (257 canónicos con grafías históricas absorbidas), y 3 snapshots `_autor` con historial combinado de las grafías duplicadas. Reutiliza sin regenerar: KMeans, mapa de temas y los 3 encoders de la 013. API: `construir_features(titulo, autor)` sigue siendo la única función del vector; coalición vigente derivada de `tabla_periodos_presidenciales.csv` por fecha. App: selector de 258 opciones. Cero librerías nuevas. Constitution Check: 8/8 principios.
- **Hechos medidos que sostienen el plan**: (1) `diputados_actuales.csv` = fuente oficial del bloque vigente (257 filas, col `Bloque`); tras unificar grafías, el bloque del último voto coincide con el oficial para los 257. (2) **48/257 diputados están en 14 bloques que el `encoder_bloque_autor` nunca vio como autores** (mayor: Provincias Unidas, 18) — el encoder ya maneja desconocidos (`unknown_value=-1`) y las features por texto conservan la señal; caso de prueba explícito. (3) **El orden de features del Escenario A NO es el orden del CSV**: STG_6.2 agrega `es_oficialista`/`coincide_bloque_autor` AL FINAL (398 features) — leer el encabezado del CSV desalinearía las columnas en silencio; por eso el orden se serializa como artefacto. (4) El único no-match nómina↔dataset es Alí "Pipi" (comillas rotas) — se resuelve quitando comillas en la normalización.
- **Anti-leakage**: snapshots acumulados hasta hoy sin `.shift` (mismo criterio aceptado en STG_8, spec 010); métrica oficial sigue siendo 0.581; el chequeo de sanidad queda documentado como no-métrica.
- **Impacto**: `specs/014-modelo-autor-en-app/plan.md` (nuevo). Próximo paso: `/tareas`.

### [2026-07-19] — Tareas 014 (APROBADAS): 17 tareas, notebook → API → app
- **Tipo**: decisión
- **Qué**: se aprobó `specs/014-modelo-autor-en-app/tasks.md`. T1-T8: notebook `STG_8.2_serializar_artefactos_autor.ipynb` (nómina canónica de 257 con fix de comillas para Alí "Pipi", snapshots con historial de grafías duplicadas combinado, `orden_features_autor.json`, reentrenamiento del LGBM ganador, chequeo de sanidad, doble corrida byte a byte). T9-T13: API (`/diputados` con 257, `construir_features(titulo, autor)`, `/predecir` exige autor, batería de casos con `TestClient` incluyendo bloque no visto por el encoder). T14-T17: selector en la app, verificación end-to-end en navegador, checklist de criterios de aceptación, registro en memoria.
- **Impacto**: `specs/014-modelo-autor-en-app/tasks.md` (nuevo). Próximo paso: `/implementar`.
...
### [2026-07-19] — Spec 015: base de datos, login y autenticación (APROBADA)
- **Tipo**: decisión
- **Qué**: se aprobó `specs/015-usuarios-login-jwt/spec.md`. Alcance: cubrir los ítems #3, #4, #5, #6, #7 del checklist MVP de la cátedra — base de datos PostgreSQL persistente en la nube (Supabase/Neon) con SQLAlchemy, registro abierto de usuarios + login, contraseñas encriptadas con bcrypt/passlib, protección de TODOS los endpoints de la API con JWT (sin token → 401), y guardado del historial de predicciones por usuario. La base guarda **solo dos tablas**: `usuarios` (usuario, contraseña encriptada, fecha alta) y `predicciones` (usuario, fecha, título, autor, resumen del resultado). Se deja un usuario de prueba fijo para el profesor.
- **Decisiones tomadas con el usuario (AskUserQuestion)**: (1) **registro abierto** desde la app + usuario de prueba fijo documentado; (2) **todo requiere login** — sin sesión activa la app solo muestra login/registro y todos los endpoints exigen token; (3) esta spec abarca solo Base de datos + Login + JWT (#3-7); la navegación multisección/gráficos (#10/12) y el deploy (#13/14/15) quedan para specs futuras (016 y 017).
- **No es una feature de ML**: no se reentrena ni se toca el modelo (spec 014); es una capa de usuarios/persistencia alrededor. `/predecir` debe devolver exactamente lo mismo que en la 014 para el mismo título+autor. Los principios P2/P3/P4 (validación temporal, anti-leakage, F1-macro) NO aplican; sí P5 (reproducibilidad), P7 (simplicidad), P8 (equipo entiende).
- **Pendiente de confirmar en /planificar o antes**: si el historial guarda solo un **resumen** de cada predicción (tema + conteo de votos) o la predicción completa de los 257 diputados. Propuesto: resumen (más liviano, alcanza para "ver mi historial").
- **Fuera de alcance**: deploy en la nube (queda todo en local contra la base en la nube), navegación multisección + gráficos, roles/permisos, recuperación de contraseña, migrar CSV a la base.
- **Impacto**: `specs/015-usuarios-login-jwt/spec.md` (nuevo). Próximo paso: `/planificar`.

### [2026-07-19] — Convención: desacople app/API respecto del modelo de predicción (vale de acá en adelante)
- **Tipo**: convención
- **Qué**: todo lo que se construya desde la spec 015 en adelante (base de datos, login, historial, frontend, deploy) debe seguir funcionando **sin cambios** si en el futuro se ajusta o reentrena el modelo — el usuario anticipó que va a querer optimizarlo para reconocer mejor los votos NEGATIVOS. La frontera única entre la aplicación y el modelo es la función `modelo.predecir_votos(titulo, autor) → (predicciones, tema_id, tema_label, bloque_autor)`, con las 3 clases de voto fijas (AFIRMATIVO/NEGATIVO/ABSTENCIÓN). Reglas concretas: (1) los módulos nuevos NO importan `api/modelo.py`; (2) el guardado del historial opera sobre el resultado ya devuelto, como datos genéricos; (3) la tabla `predicciones` guarda conceptos del dominio político (título, autor, tema, conteo de votos), no artefactos internos del modelo (features, pesos, columnas, versiones); (4) optimizar el modelo = reemplazar el `.joblib` + lógica en `modelo.py`, sin tocar ningún archivo de la capa de app. Solo un cambio que rompa el contrato (ej. una 4ª clase de voto, o un 3er dato de entrada) obligaría a tocar `modelo.py` y el router `/predecir`, nunca la capa de usuarios/base.
- **Por qué**: el usuario quiere poder iterar el modelo (en especial mejorar la detección de NEGATIVOS, la clase más difícil) sin romper la app ni la base de datos. Desacoplar por contrato evita reescrituras en cascada cada vez que cambie el modelo.
- **Impacto**: quedó documentado como sección "Independencia del modelo de predicción" en `specs/015-usuarios-login-jwt/plan.md`. Verificable con una búsqueda de imports (ningún módulo de auth/DB importa `modelo.py`). Aplica a todas las specs futuras.

### [2026-07-19] — Plan 015 (APROBADO): capa de auth/DB alrededor de la API, con Supabase
- **Tipo**: decisión
- **Qué**: se aprobó `specs/015-usuarios-login-jwt/plan.md`. Diseño: módulos nuevos en `api/` sin renombrar los existentes — `db.py` (conexión SQLAlchemy a PostgreSQL, distinta del `database.py` de los CSV), `tablas.py` (ORM `Usuario` y `Prediccion`), `seguridad.py` (bcrypt vía passlib + JWT vía python-jose + dependencia `usuario_actual`), `routers/auth.py` (`/registro`, `/login`) y `routers/historial.py` (`/mis-predicciones`). Se protegen con JWT los endpoints existentes (`/diputados`, `/diputados/{id}`, `/predecir`) y `/predecir` guarda un resumen de cada predicción (título, autor, tema, conteo de votos) asociado al usuario. Tablas creadas con `create_all` al iniciar (sin Alembic, P7). Usuario de prueba sembrado idempotentemente al arrancar, con credenciales de `.env`. Frontend mínimo: guardián de sesión con `st.session_state`, login/registro, tabla "Mis predicciones", cerrar sesión.
- **Proveedor de base elegido**: **Supabase** (PostgreSQL gratuito en la nube). La cadena de conexión va en `.env` (`DATABASE_URL`), nunca en el código.
- **Librerías nuevas (5)**: `SQLAlchemy`, `psycopg2-binary`, `passlib[bcrypt]`, `python-jose[cryptography]`, `python-dotenv`. Riesgo anotado: fijar `bcrypt==4.0.1` (las ≥4.1 rompen con passlib 1.7.4).
- **Constitution Check**: pasa los 8 principios. P2/P3/P4 (validación temporal, anti-leakage, F1-macro) N/A porque no se entrena ni toca ningún modelo — es infraestructura. Cuidado equivalente: `/predecir` debe devolver idéntico a la spec 014 (se verifica).
- **Deploy fuera de alcance** (spec 017): esta spec deja todo corriendo en local contra la base Supabase en la nube.
- **Impacto**: `specs/015-usuarios-login-jwt/plan.md` (aprobado). Próximo paso: `/tareas`.

### [2026-07-19] — Tareas 015 (APROBADAS) + avance T2, T6, T7, T9 (independientes de Supabase)
- **Tipo**: decisión
- **Qué**: se aprobó `specs/015-usuarios-login-jwt/tasks.md` (21 tareas). Se implementaron las 4 tareas que no dependen de tener la cuenta de Supabase creada, mientras el usuario la crea en paralelo:
  - **T2**: instaladas y fijadas en `requirements.txt`: `SQLAlchemy==2.0.43`, `psycopg2-binary==2.9.10`, `passlib==1.7.4`, `bcrypt==4.0.1`, `python-jose==3.3.0` (+ `cryptography==46.0.3`), `python-dotenv==1.1.0`. `SQLAlchemy`, `bcrypt` (5.0.0) y `python-dotenv` ya estaban instalados como dependencias de otra cosa; se bajó `bcrypt` a 4.0.1 por el pin de compatibilidad con passlib ya anotado en el plan. Verificado: `bcrypt==4.0.1` tiene wheel `cp36-abi3` (ABI estable), corre en Python 3.13 sin compilar desde fuente.
  - **T6/T7**: `api/seguridad.py` nuevo — `hashear_clave`/`verificar_clave` (passlib+bcrypt) y `crear_token`/`validar_token` (JWT vía python-jose, HS256, vencimiento 12h). Probado a mano: hash no contiene la clave en texto plano, verifica correcta/rechaza incorrecta, token válido se acepta, token manipulado y token vencido se rechazan con `TokenInvalidoError`.
  - **T9**: agregados a `api/schemas.py`: `RegistroRequest` (username 3-50 char, password mín. 6), `LoginRequest`, `TokenResponse`, `PrediccionGuardada`. Probado: rechaza username vacío y password corta.
  - **Adelanto parcial de T3**: se creó `.env` (con `JWT_SECRET_KEY` generado con `secrets.token_hex(32)`, y `USUARIO_PRUEBA`/`CLAVE_PRUEBA` de prueba) y `.env.example` sin valores reales. `DATABASE_URL` quedó como placeholder `PENDIENTE_COMPLETAR_CON_LA_CADENA_DE_SUPABASE` hasta que el usuario cree la cuenta Supabase (T1). Verificado con `git status`: `.env` no aparece (ignorado), `.env.example` sí.
- **Por qué**: el usuario quería avanzar en paralelo a crear la cuenta de Supabase; estas 4 tareas (más la porción de T3 sin `DATABASE_URL`) no dependen de tener la base creada.
- **Impacto**: `api/seguridad.py` (nuevo), `api/schemas.py` (modificado), `requirements.txt` (modificado), `.env` y `.env.example` (nuevos). Pendiente: completar `DATABASE_URL` en `.env` en cuanto el usuario tenga la cadena de Supabase, para continuar con T1/T3 completa → T4 (conexión SQLAlchemy).

### [2026-07-19] — Spec 015, T1/T3/T4: conexión real a Supabase establecida
- **Tipo**: decisión
- **Qué**: el usuario creó el proyecto Supabase (`Legistrack 2026`, región `sa-east-1`). Se usó el **Transaction pooler** (puerto 6543, host `aws-1-sa-east-1.pooler.supabase.com`) en vez de la conexión directa, porque la conexión directa de Supabase gratis requiere IPv6 (muchas redes hogareñas no lo tienen) y el pooler funciona por IPv4 desde cualquier red. `api/db.py` creado: motor SQLAlchemy con `pool_pre_ping=True` (evita errores por conexiones que el pooler cierra por inactividad), `SessionLocal`, `Base` declarativa y la dependencia `get_db`. Conexión probada de punta a punta con `SELECT 1` real contra Supabase: OK.
- **Por qué / detalle técnico**: la contraseña elegida por el usuario contenía caracteres especiales (`@` y `!`) que rompen el formato de URI de conexión si van sin codificar (un `@` extra en la cadena confunde dónde termina la contraseña y empieza el host). Se codificaron con `urllib.parse.quote(clave, safe='')` → `@`→`%40`, `!`→`%21`, antes de armar `DATABASE_URL`. **Lección para el equipo**: si en el futuro se cambia la contraseña de la base, y tiene caracteres especiales, hay que codificarlos igual antes de pegarlos en `DATABASE_URL` (los signos seguros sin codificar: letras, números, `-`, `_`, `.`, `~`).
- **Impacto**: `api/db.py` (nuevo), `.env` completo con `DATABASE_URL` real (no se commitea). Próximo paso: T5 (tablas `Usuario`/`Prediccion`).

### [2026-07-19] — Spec 015, T5: tablas `usuarios`/`predicciones` creadas en Supabase
- **Tipo**: decisión
- **Qué**: `api/tablas.py` nuevo con los modelos ORM `Usuario` (id, username único, password_hash, fecha_alta) y `Prediccion` (id, usuario_id → FK, fecha, titulo, autor, tema, n_afirmativo, n_negativo, n_abstencion), con relación `usuario.predicciones` en ambos sentidos. `Base.metadata.create_all(engine)` corrido contra Supabase real: ambas tablas se crearon con todas las columnas esperadas (verificado con `sqlalchemy.inspect`). Prueba adicional de round-trip: insertar un usuario + una predicción, leer la predicción vía `usuario.predicciones` (relación funciona), y limpiar los datos de prueba antes de seguir (la base quedó vacía de nuevo).
- **Por qué**: verificar la relación ORM ahora, antes de construirle encima los routers de T10-T14, para no descubrir un problema de relación recién al integrar.
- **Impacto**: `api/tablas.py` (nuevo). Tablas `usuarios` y `predicciones` ya existen en el proyecto Supabase `Legistrack 2026`. Próximo paso: T8 (dependencia `usuario_actual`, ya puede usar `Usuario` real) o T10 (`POST /registro`).

### [2026-07-19] — Spec 015, T8: dependencia `usuario_actual` protege endpoints con JWT real
- **Tipo**: decisión
- **Qué**: se agregó a `api/seguridad.py` la dependencia FastAPI `usuario_actual(credenciales, db)` — lee el token del encabezado `Authorization: Bearer` con `HTTPBearer(auto_error=False)`, lo valida con `validar_token` (T7), busca el `Usuario` real en la base (T5) por `username`, y devuelve el objeto `Usuario`. Responde 401 uniforme (`"No autorizado"`, header `WWW-Authenticate: Bearer`) ante cualquiera de estos casos: falta el token, token inválido/manipulado, token vencido, o el usuario del token ya no existe en la base (ej. se borró la cuenta pero el token viejo sigue circulando).
- **Validado**: probado con un mini-servidor FastAPI + `TestClient` y una ruta protegida de prueba, 4 casos: sin token (401), token manipulado (401), token válido de un usuario inexistente (401), token válido de un usuario real creado en la base (200, devuelve el username correcto). Usuario de prueba limpiado de la base al terminar.
- **Impacto**: `api/seguridad.py` (modificado). Con esto quedan resueltas las piezas de bajo nivel de auth (T5, T6, T7, T8); lo que sigue son los endpoints que las usan: T10 (`/registro`), T11 (`/login`), T12 (`/mis-predicciones`), T13 (proteger `/diputados`, `/diputados/{id}`, `/predecir`).

### [2026-07-19] — Spec 015, T10: `POST /registro` funcionando
- **Tipo**: decisión
- **Qué**: `api/routers/auth.py` nuevo con `POST /registro` (201 al crear, 400 si el username ya existe). Usa `hashear_clave` (T6) antes de guardar y la validación de `RegistroRequest` (T9, username 3-50 char, password mín. 6).
- **Validado**: probado con `TestClient` real contra Supabase — registro nuevo (201), username duplicado (400), username muy corto rechazado por Pydantic (422 automático, sin código extra), y confirmado que `password_hash` en la base NO contiene la clave en texto plano. Usuario de prueba limpiado.
- **Impacto**: `api/routers/auth.py` (nuevo, con `POST /registro`; `POST /login` se agrega en T11 al mismo archivo/router). Próximo paso: T11 (`POST /login`).

### [2026-07-19] — Spec 015, T11: `POST /login` funcionando
- **Tipo**: decisión
- **Qué**: se agregó `POST /login` a `api/routers/auth.py`. Valida usuario+clave (`verificar_clave`, T6) y devuelve `TokenResponse` con un JWT (`crear_token`, T7). Ante usuario inexistente o clave incorrecta, devuelve el **mismo mensaje genérico** "Usuario o clave incorrectos" (401) — decisión de seguridad para no revelarle a un atacante cuál de las dos cosas falló (no confirma ni niega la existencia de un username).
- **Validado**: probado con `TestClient` — login correcto devuelve un token que, al pasarlo por `validar_token`, resuelve al username correcto; clave incorrecta → 401; usuario inexistente → 401 (mismo mensaje que el caso anterior). Usuario de prueba limpiado.
- **Impacto**: `api/routers/auth.py` (modificado). Próximo paso: T12 (`GET /mis-predicciones`).

### [2026-07-19] — Spec 015, T12: `GET /mis-predicciones` con aislamiento por usuario verificado
- **Tipo**: decisión
- **Qué**: `api/routers/historial.py` nuevo con `GET /mis-predicciones` (protegido por `usuario_actual`), filtra `Prediccion` por `usuario_id == usuario.id` y ordena por `fecha` descendente.
- **Validado**: prueba con **dos usuarios reales** y predicciones cruzadas — usuario A (2 predicciones) ve solo las suyas en el orden correcto (más reciente primero), usuario B (1 predicción) ve solo la suya, ninguno ve las del otro. Sin token → 401. Datos de prueba limpiados.
- **Impacto**: `api/routers/historial.py` (nuevo). Con esto quedan completos todos los endpoints nuevos de auth/historial (T9-T12). Próximo paso: T13 (proteger `/diputados`, `/diputados/{id}`, `/predecir` con `usuario_actual`) y T14 (guardar cada predicción real en `/predecir`).

### [2026-07-19] — Spec 015, T13: endpoints existentes (spec 010/014) protegidos con JWT
- **Tipo**: decisión
- **Qué**: `api/routers/diputados.py` y `api/routers/predecir.py` ahora exigen `usuario_actual` — se aplicó a nivel de **router completo** (`APIRouter(..., dependencies=[Depends(usuario_actual)])`) en vez de endpoint por endpoint, porque ninguna de las funciones necesita todavía el objeto `Usuario` (eso recién se usa en T14 para `/predecir`, donde ahí sí se cambia a un parámetro explícito de la función).
- **Validado**: `/diputados`, `/diputados/{id}` y `/predecir` dan 401 sin token. Con token válido, comportamiento **idéntico** a antes de esta spec: 257 diputados, historial real de un diputado, y `/predecir` devuelve 257 predicciones (probado con un título y autor reales, incluyendo la carga real del modelo LGBM + embedder). Este es el primer chequeo de la garantía de "independencia del modelo" del plan: agregar auth no cambió nada del resultado del modelo.
- **Impacto**: `api/routers/diputados.py`, `api/routers/predecir.py` (modificados). Próximo paso: T14 (guardar cada predicción real en la tabla `predicciones`, usando ya el `Usuario` de `usuario_actual` como parámetro de la función en vez de solo como dependencia del router).

### [2026-07-19] — Spec 015, T14: cada predicción queda guardada en la base, ligada al usuario
- **Tipo**: decisión
- **Qué**: `api/routers/predecir.py` — la dependencia `usuario_actual` pasó de estar declarada a nivel de router (`dependencies=[...]`) a ser un parámetro explícito de la función `predecir()`, porque ahora sí se necesita el objeto `Usuario` para guardar la fila (FastAPI cachea la dependencia por request, así que no se evalúa dos veces; tener las dos formas a la vez era redundante y se limpió). Nueva función `_guardar_historial()`: cuenta los votos de la lista que YA devolvió `modelo.predecir_votos` (mira únicamente la clave `voto_predicho` de cada dict, dato genérico) e inserta una fila en `predicciones` con título, autor, tema y el conteo de AFIRMATIVO/NEGATIVO/ABSTENCIÓN. Cumple al pie la letra el principio de independencia del modelo (plan 015): esta función no importa nada interno de `modelo.py` más allá del resultado ya calculado.
- **Detalle de datos verificado**: las etiquetas reales que devuelve el modelo (`le_voto_autor.joblib`) son `'AFIRMATIVO'`, `'NEGATIVO'`, `'ABSTENCIÓN'` (con tilde, Unicode U+00D3 correcto — lo que parecía un caracter roto era solo la terminal de Windows mostrándolo mal, no un bug de datos). El conteo usa `.get("ABSTENCION", .get("ABSTENCIÓN", 0))`, mismo patrón defensivo que ya usaban `app.py`/`database.py`.
- **Validado**: predicción real con token → 200, se insertó una fila en `predicciones` cuyo conteo (257/0/0 para el caso de prueba) coincide exactamente con el conteo calculado a partir de la respuesta HTTP. Sin token sigue dando 401. Datos de prueba limpiados.
- **Impacto**: `api/routers/predecir.py` (modificado). Con esto quedan completas todas las tareas de backend (T4-T15 salvo T15 mismo). Próximo paso: T15 (cablear `main.py`: registrar routers `auth`/`historial`, `create_all` + sembrar usuario de prueba en el `lifespan`).

### [2026-07-19] — Spec 015, T15: `api/main.py` cableado — backend completo funcionando con `uvicorn` real
- **Tipo**: decisión
- **Qué**: `api/main.py` ahora: (1) llama `load_dotenv()` **antes** de importar cualquier módulo de `api/` (crítico: `api/db.py` lee `DATABASE_URL` al importarse, a nivel de módulo, para crear el `engine`); (2) en el `lifespan`, corre `Base.metadata.create_all(engine)` (crea `usuarios`/`predicciones` si no existen) y siembra el usuario de prueba de forma idempotente (`_sembrar_usuario_prueba()`, lee `USUARIO_PRUEBA`/`CLAVE_PRUEBA` del `.env`, no hace nada si ya existe); (3) registra los 4 routers (`auth`, `diputados`, `predecir`, `historial`).
- **Validado**: (a) idempotencia probada llamando `_sembrar_usuario_prueba()` dos veces seguidas — el usuario "profesor" queda exactamente 1 vez en la tabla; (b) servidor `uvicorn` real levantado (puerto 8001, el 8000 estaba ocupado por otro proceso preexistente que no se tocó): login del usuario de prueba → 200 + token; `GET /diputados` con ese token → 200, 257 diputados; `GET /mis-predicciones` → 200, lista vacía (el usuario de prueba recién sembrado no tiene predicciones); `GET /diputados` sin token → 401.
- **Nota operativa**: el puerto 8000 (el que usa `.claude/launch.json` para la API) estaba ocupado por un proceso preexistente (PID distinto al de esta sesión) durante la prueba; se usó el puerto 8001 para no arriesgarse a matar un proceso ajeno sin identificarlo primero. Si en T19 (prueba de punta a punta con Streamlit) el puerto 8000 sigue ocupado, hay que identificar y liberar ese proceso antes de levantar el servidor real ahí.
- **Impacto**: `api/main.py` (modificado). **Con esto, todo el backend de la spec 015 está completo y probado de punta a punta** (T1-T15). Queda el frontend (T16-T18) y el cierre (T19-T21).

### [2026-07-19] — Spec 015, T16: guardián de sesión + login/registro en `app/app.py`
- **Tipo**: decisión
- **Qué**: `app/app.py` — nueva función `_pantalla_login()` con dos pestañas (`st.tabs`): "Iniciar sesión" (llama `POST /login`, guarda `access_token`/`username` en `st.session_state` y hace `st.rerun()`) y "Registrarse" (llama `POST /registro`). El guardián `if "token" not in st.session_state: _pantalla_login(); st.stop()` se ubica **antes** de cualquier llamada a la API de diputados/predicción — sin sesión, ninguna de esas funciones se ejecuta, no solo queda oculta visualmente.
- **Validado** con `streamlit.testing.v1.AppTest` (harness oficial de Streamlit para testear apps sin navegador) contra el servidor `uvicorn` real: (a) sin sesión, solo se ven las 2 pestañas de login/registro, cero excepciones, `token` ausente de `session_state`, y **no aparece el selector de diputado** (confirma que el resto de la app no se ejecuta); (b) login con el usuario de prueba real (`profesor`/`legistrack2026`) → `token` queda en `session_state` con el `username` correcto; (c) login con clave incorrecta → `token` NO se guarda, se muestra el error "Usuario o contraseña incorrectos"; (d) registro de un usuario nuevo desde la pantalla → mensaje de éxito, usuario creado en la base (limpiado después de la prueba).
- **Nota operativa**: el servidor `uvicorn` de prueba del puerto 8001 (levantado en T15) seguía corriendo — un intento anterior de `kill` no afectó al proceso real (en Git Bash sobre Windows, el PID que devuelve `$!` para un proceso en segundo plano no siempre coincide con el PID real de Python/uvicorn). Se reutilizó sin problema; sigue arriba para las pruebas de T17/T18/T19.
- **Impacto**: `app/app.py` (modificado). Próximo paso: T17 (mandar el token en cada request + botón de cerrar sesión) — hasta que esa tarea esté hecha, entrar a la app tras el login mostraría error de conexión al llamar a `/diputados` sin el header `Authorization` (esperado, es la tarea siguiente).

### [2026-07-19] — Spec 015, T17: token en cada request + cerrar sesión, app funcionando completa
- **Tipo**: decisión
- **Qué**: `app/app.py` — helper `_headers_auth()` que arma `{"Authorization": f"Bearer {st.session_state['token']}"}`. Se agregó como parámetro `_headers` (prefijo `_`, la convención de Streamlit para excluir un argumento no-hasheable de la clave de `@st.cache_data` — un dict no se puede hashear y ademas no queremos que el token invalide el cache) a `listar_diputados()` y `consultar_historial()`; `predecir_votos()` (sin cache) llama `_headers_auth()` directo. Se agregó una fila con "Sesión iniciada como **{username}**" y un botón "Cerrar sesión" (borra `token`/`username` de `session_state` y hace `st.rerun()`), ubicados justo después del guardián de T16.
- **Validado** con `AppTest` contra el servidor real: (a) login exitoso → la app entra de verdad ahora (antes fallaría): selector de diputado con 257 opciones, sin excepciones; (b) "Consultar" un diputado real → historial se muestra sin excepciones (confirma que `consultar_historial` también manda el token); (c) botón "Cerrar sesión" → `token` desaparece de `session_state` y la app vuelve a mostrar las pestañas de login/registro.
- **Impacto**: `app/app.py` (modificado). Con esto la app queda **completamente funcional de punta a punta** (login → consulta → predicción → logout). Falta T18 (tabla "Mis predicciones") y el cierre (T19-T21).

### [2026-07-19] — Spec 015, T18: tabla "Mis predicciones" en la app
- **Tipo**: decisión
- **Qué**: sección nueva al final de `app/app.py` que llama `GET /mis-predicciones` (sin `@st.cache_data` a propósito, para traer siempre la lista más reciente en cada rerun) y muestra una tabla (Fecha, Proyecto, Autor, Tema, AFIRMATIVO, NEGATIVO, ABSTENCIÓN). Si el usuario no tiene predicciones todavía, muestra un mensaje en vez de una tabla vacía.
- **Validado** con `AppTest`, flujo completo con un usuario nuevo real: (a) recién registrado y logueado, sin predicciones previas, se ve el mensaje de estado vacío; (b) tras predecir un título real, la nueva fila aparece de inmediato en la tabla (sin recargar la página a mano — el mismo rerun que muestra el resultado de la predicción ya trae la tabla actualizada, porque no hay cache de por medio); columnas y título coinciden. Usuario y predicción de prueba limpiados de la base.
- **Impacto**: `app/app.py` (modificado). **Con esto quedan completas todas las tareas de construcción de la spec 015 (T1-T18).** Falta el cierre: T19 (prueba de punta a punta + comparación exacta contra la spec 014), T20 (checklist de criterios de aceptación) y T21 (registro final en memoria).

### [2026-07-19] — Spec 015, T19: prueba de punta a punta + verificación bit a bit contra el modelo sin capa de auth
- **Tipo**: decisión
- **Qué**: dos pruebas finales contra un servidor `uvicorn` real (puerto 8001) usando `AppTest` para la parte de Streamlit:
  1. **Flujo completo de un usuario real**, en un solo recorrido continuo: sin sesión → solo login/registro visible (0 selectbox) → registro → login (257 diputados cargados) → "Consultar" un diputado real (historial se muestra) → "Predecir" un título real → la predicción aparece de inmediato en "Mis predicciones" → "Cerrar sesión" (vuelve a login) → confirmar que `/diputados` y `/predecir` dan 401 sin token en la API real. **Los 8 pasos pasaron sin excepciones.**
  2. **Verificación de independencia del modelo** (la garantía central del plan 015): se llamó a `modelo.predecir_votos(titulo, autor)` **directamente**, sin pasar por ninguna capa nueva (equivalente a como funcionaba en la spec 014), y por separado se llamó a `POST /predecir` a través de toda la pila nueva (JWT + guardado en base). Mismo título, mismo autor. Resultado: **las 257 predicciones (diputado, bloque, voto_predicho) son bit a bit idénticas** entre ambos caminos, además de tema y bloque_autor idénticos. Confirma empíricamente que envolver `/predecir` con autenticación y persistencia no alteró en absolutamente nada el resultado del modelo.
- **Impacto**: datos de prueba (`_test_t19_e2e` y sus 2 predicciones) limpiados de Supabase; servidor `uvicorn` de prueba (PID 8308, venía de T15) detenido tras verificar con `tasklist` que era el proceso correcto antes de matarlo. **La spec 015 queda funcionalmente completa y verificada de punta a punta.** Próximo paso: T20 (checklist formal de los 11 criterios de aceptación de `spec.md`).

### [2026-07-19] — Spec 015, T20: 11/11 criterios de aceptación marcados con evidencia
- **Tipo**: decisión
- **Qué**: se marcaron los 11 criterios de `specs/015-usuarios-login-jwt/spec.md` con evidencia concreta (tarea + prueba que lo verificó), reusando lo ya probado en T1-T19. Reverificación puntual hecha en el momento (no solo recordada): `git status` confirmó que `.env` sigue sin trackear y `.env.example` sí, después de todos los cambios de la spec.
- **Caveat explícito en el criterio del usuario de prueba**: el mecanismo (sembrado idempotente al iniciar la API, leído de `.env`) está construido y probado, pero hoy solo está "documentado" para el equipo (`.env.example` + memoria). Una nota en el README con las credenciales reales, visible para el profesor, tiene sentido recién **junto con el deploy** (spec 017, que es donde vive el ítem #18 del checklist de la cátedra — README + credenciales de prueba). Se marcó igual como cumplido porque el criterio de la spec 015 es sobre el mecanismo, no sobre la documentación pública de producción (explícitamente fuera de alcance de esta spec).
- **Impacto**: `specs/015-usuarios-login-jwt/spec.md` (checklist actualizado, 11/11). Próximo paso: T21 (cierre de la spec en memoria) y luego decidir con el usuario el punto pendiente de replicar todo esto en el repo espejo (ver convención [repo espejo]).

### [2026-07-19] — Spec 015 COMPLETA: base de datos, login y autenticación (usuarios + historial de predicciones)
- **Tipo**: decisión
- **Qué**: se cerraron las 21 tareas de `specs/015-usuarios-login-jwt/tasks.md` y los 11 criterios de aceptación de `spec.md` (cubre los ítems #3, #4, #5, #6, #7 del checklist MVP de la cátedra). Resultado:
  - **Base de datos**: proyecto **Supabase** (`Legistrack 2026`, región `sa-east-1`), conectado vía **Transaction pooler** (puerto 6543, no la conexión directa — Supabase gratis exige IPv6 para conexión directa y muchas redes hogareñas no lo tienen). Dos tablas (`usuarios`, `predicciones`), creadas solas al iniciar la API (`create_all`, sin Alembic).
  - **Auth**: `POST /registro`, `POST /login` (JWT, 12h de vencimiento), contraseñas con `bcrypt`/`passlib`. Todos los endpoints existentes (`/diputados`, `/diputados/{id}`, `/predecir`) y los nuevos (`/mis-predicciones`) exigen token válido (401 si no).
  - **Historial**: cada predicción se guarda automáticamente (tema + conteo AFIRMATIVO/NEGATIVO/ABSTENCIÓN) asociada al usuario; aislamiento entre usuarios verificado con datos reales.
  - **Usuario de prueba**: sembrado idempotente al iniciar la API, credenciales en `.env`/`.env.example`.
  - **Frontend**: `app/app.py` con guardián de sesión (`st.session_state`), pantalla de login/registro, token en cada request, botón de cerrar sesión, tabla "Mis predicciones".
  - **Independencia del modelo (garantía central del plan)**: verificada empíricamente en T19 — `modelo.predecir_votos()` llamado directo vs. `POST /predecir` autenticado dan **257 predicciones bit a bit idénticas**. Ningún módulo de auth/DB importa `api/modelo.py`.
- **Bugs/lecciones encontrados en el camino** (causa raíz, no solo síntoma):
  1. **`passlib` + `bcrypt` recientes son incompatibles** (bcrypt ≥4.1 rompe la detección de versión de passlib 1.7.4). Se fijó `bcrypt==4.0.1` (tiene wheel `cp36-abi3`, corre en Python 3.13 sin compilar). Riesgo anticipado en el plan, confirmado y mitigado antes de que causara un fallo real.
  2. **Contraseñas con caracteres especiales rompen la URI de conexión**: la contraseña de Supabase tenía `@` y `!`, que confunden el parseo de `postgresql://user:pass@host`. Se codificaron con `urllib.parse.quote(clave, safe='')` (`@`→`%40`, `!`→`%21`) antes de armar `DATABASE_URL`. Aplica a cualquier contraseña futura con caracteres especiales.
  3. **Dependencia duplicada en `/predecir`**: al principio `usuario_actual` estaba tanto a nivel de router como parámetro de la función (T13 y T14 respectivamente). Se limpió dejando solo el parámetro de función (T14), ya que FastAPI cachea la dependencia por request — tenerla dos veces era redundante, no un bug funcional, pero sí código de más.
  4. **Caracteres acentuados "rotos" en la salida de la terminal de Windows** (ej. `ABSTENCI�N`, `Iniciar sesi�n`): en todos los casos verificado con `repr()`/hex de los caracteres que el dato real está bien codificado en UTF-8 — es solo la consola de Windows mostrándolo mal. **Lección para el equipo**: no asumir que un `�` en la salida de un comando es un bug de datos sin verificar primero con `repr()`.
  5. **PID de `$!` en Git Bash no coincide con el PID real del proceso en Windows**: un intento de `kill` sobre el PID que devolvía `$!` no mató el proceso real de `uvicorn`; hubo que buscar el PID real con `netstat`/`tasklist` antes de matarlo. Lección operativa para futuras pruebas con servidores en segundo plano en este entorno.
- **Validado (T19-T20)**: prueba de punta a punta completa (registro → login → consulta → predicción → historial propio → logout → 401 sin token) sin excepciones; los 11 criterios de `spec.md` marcados con evidencia.
- **Caveat pendiente, no bloqueante**: la documentación pública del usuario de prueba (README con credenciales reales para el profesor) queda para la spec 017 (deploy), donde tiene sentido real (URL pública + credenciales de producción).
- **Impacto**: nuevos: `api/db.py`, `api/tablas.py`, `api/seguridad.py`, `api/routers/auth.py`, `api/routers/historial.py`, `.env` (no versionado), `.env.example`, `specs/015-usuarios-login-jwt/` (spec, plan, tasks — completos). Modificados: `api/main.py`, `api/routers/diputados.py`, `api/routers/predecir.py`, `api/schemas.py`, `app/app.py`, `requirements.txt`. Tablas nuevas en Supabase: `usuarios`, `predicciones`. **Próximo paso**: correr `/revisar`. Recordar al usuario la guía pendiente para replicar esta spec en el repo espejo (ver [[project_repo_espejo]] / convención registrada el 2026-07-19).

### [2026-07-19] — Spec 015: `/revisar` con sign-off + 4 hallazgos corregidos
- **Tipo**: bug
- **Qué**: `/revisar` dio sign-off a la spec 015 (11/11 criterios, Constitución 8/8) con 4 hallazgos de robustez, no bloqueantes, corregidos de inmediato a pedido del usuario:
  1. **Condición de carrera en `POST /registro`** (`api/routers/auth.py`): el chequeo de "¿existe?" y el `db.commit()` eran dos pasos separados — si dos registros con el mismo username llegaban casi simultáneos, el segundo podía recibir un 500 feo (`IntegrityError`) en vez de un 400 prolijo. Se envolvió el `commit()` en `try/except IntegrityError` → `db.rollback()` + 400. **Causa raíz**: patrón clásico "check-then-act" sin protección atómica; la restricción `unique=True` de la tabla sí evitaba el dato duplicado, pero no daba una respuesta HTTP limpia. Verificado reproduciendo la carrera real con dos sesiones SQLAlchemy que "ven" `None` antes de que cualquiera comitee: la segunda sesión sí dispara `IntegrityError` (la misma excepción que ahora atrapa el endpoint). Regresión confirmada: registro nuevo (201) y duplicado vía el chequeo previo (400) siguen iguales.
  2. **El caché de Streamlit no volvía a validar el token una vez tibio** (`app/app.py`, `listar_diputados`/`consultar_historial`): como el header se excluye de la clave de `@st.cache_data` (un dict no es hasheable), una vez cacheado un dato exitosamente, nunca se volvía a golpear la API para esa función — así que un token vencido (12hs) podía seguir "funcionando" para datos ya cacheados, sin fuga de datos sensibles (es información pública de todas formas) pero sin re-chequear la sesión. **Causa raíz**: tensión entre caching y frescura de sesión; nunca se puso un `ttl`. Se agregó `ttl=1800` (30 min, bien por debajo de las 12hs del token) a ambas funciones cacheadas. Verificado: la app sigue funcionando igual (257 diputados, sin excepciones) tras el cambio.
  3. **Sin `max_length` en la contraseña + límite silencioso de `bcrypt`** (`api/schemas.py`): `bcrypt` ignora en silencio todo lo que pase el byte 72 de una clave (confirmado en vivo: una clave de 100 caracteres y esa misma clave cortada a 72 verifican igual). Se agregó `max_length=72` a `RegistroRequest.password`. Verificado: rechaza 100 caracteres, acepta exactamente 72 (borde).
  4. **El validador de username no re-chequeaba el largo mínimo después de limpiar espacios** (`api/schemas.py`): un username como `" ab"` (espacio + 2 letras = 3 caracteres crudos) pasaba el `min_length=3` de Pydantic *antes* de limpiarse, y quedaba guardado como `"ab"` (2 caracteres) sin volver a validar. Se cambió el validador para chequear `len(v) < 3` **después** de `.strip()`. Verificado: rechaza `" ab"`, acepta `" abc "` → `"abc"`.
- **Impacto**: `api/routers/auth.py`, `api/schemas.py`, `app/app.py` (3 cambios chicos). **Spec 015 queda completa, revisada y sin observaciones pendientes.** Próximo paso: réplica en el repo espejo (decisión pendiente: misma base Supabase vs. una nueva — el usuario ya indicó que prefiere la misma).

### [2026-07-19] — Repo espejo: replicar a mano los pasos NO versionados de la spec 015
- **Tipo**: convención
- **Qué**: el usuario mantiene un **repositorio espejo** del proyecto (en otra máquina/carpeta) donde NO usa Claude Code. Al cerrar cada spec, copia y pega los cambios de código al espejo. Pero varios pasos de la spec 015 **no viajan** con el copiar/pegar de código, porque son locales o manuales — hay que reproducirlos a mano en el espejo, y el usuario pidió que Claude lo guíe paso a paso cuando llegue ese momento. Pasos a replicar en el espejo al terminar la spec 015:
  1. **Instalar las librerías nuevas** (T2) en el entorno de Python del espejo: `SQLAlchemy`, `psycopg2-binary`, `passlib==1.7.4`, `bcrypt==4.0.1` (pin), `python-jose`, `python-dotenv`, `cryptography` — están en `requirements.txt`, así que un `pip install -r requirements.txt` alcanza.
  2. **Crear el archivo `.env`** (no está versionado, `.gitignore` lo bloquea): `DATABASE_URL`, `JWT_SECRET_KEY`, `USUARIO_PRUEBA`, `CLAVE_PRUEBA`. Existe `.env.example` como plantilla.
  3. **Supabase**: decidir si el espejo apunta a la MISMA base Supabase (mismo `DATABASE_URL` → las tablas ya existen) o a un proyecto Supabase nuevo (habría que recrear proyecto + tablas con `create_all`). Pendiente de definir con el usuario.
  4. **Codificar la contraseña** de la cadena de conexión si tiene caracteres especiales (mismo tema `@`→`%40`, `!`→`%21` que ya resolvimos; usar `urllib.parse.quote(clave, safe='')`).
- **Por qué**: sin esta lista, al pasar la spec al espejo el código no arrancaría (faltarían dependencias, `.env` y la base) y el usuario no tiene Claude Code de ese lado para diagnosticarlo.
- **Impacto**: recordatorio operativo. Al cerrar la spec 015 (después de T21), Claude debe ofrecer una guía paso a paso de estos 4 puntos para el repo espejo.
