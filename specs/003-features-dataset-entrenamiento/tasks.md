...
- [x] T2 — Calcular embeddings con `paraphrase-multilingual-MiniLM-L12-v2` e imprimir shape de la matriz resultante (depende de: T1)
- [x] T3 — Correr K-Means con K=10, K=15 y K=20 (`random_state=42`); para cada K mostrar tamaño de grupos y los 5 títulos más cercanos al centro de cada grupo (depende de: T2)
- [x] T4 — Generar etiquetas automáticas por grupo: 5 palabras más frecuentes de los títulos del grupo, excluyendo stopwords en español + palabras legislativas vacías ("ley", "nacional", "artículo", "modificación") (depende de: T3)
- [x] T5 — Celda de elección de K: fijar `K_ELEGIDO=20` y recalcular clustering final con ese K (depende de: T4)
